...
With `algorithm = "skip"` the eta-count stage is left out and only the block stage runs, which is the cheap way to ask one question: should these two etas be correlated?

```{r}
#| label: iivsearch-blocks
iiv_block <- ferx_iivsearch(model = bo$model, data = bo$data,
                            search_space = "COVARIANCE?(IIV, [ETA_CL, ETA_V])",
                            algorithm = "skip", block_retries = 1,
                            directory = book_tempdir("iivsearch-block"), progress = FALSE)
iiv_block$final_structure
```
