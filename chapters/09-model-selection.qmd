...
ferx-core also has variability searches (IIV and IOV structures), an automatic model-development pipeline and a global search. They run from the ferx command line and have no R function in the pinned ferx-r build. See the ferx-core [tools](https://ferx-nlme.github.io/ferx-core/tools/index.html) pages.
...
The three search functions share the entry arguments `model`, `data` and `config`; the run arguments `threads`, `retries`, `directory`, `resume` and `progress`; and for model and covariate search `search_space`, `rank` and `cutoff`. A search is stated either by `config` or by the inline arguments, not both:
...
`print()` and `summary()` exist for the results of all three: `ferx_covsearch`, `ferx_modelsearch` and `ferx_ruvsearch` objects. `ferx_search_config` and `ferx_search_space` objects have `print()` methods.
