...
The same `W_DESIGN_DV` warning appears, for the same reason: the design rows have an empty `DV`, so ferx simulates them as design points. It is again recorded in `attr(sim, "simulation_warnings")`.
