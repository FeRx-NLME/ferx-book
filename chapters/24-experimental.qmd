...
Three parts of ferx are explicitly experimental: stochastic differential equations, which add random noise to the model states between observations; neural networks for the covariate model, known as deep compartment models; and parameter priors, which regularize a fit towards values you already believe. The first two fit from R with the bundled examples. This chapter runs them, compares them with their conventional counterparts, and shows what the fits report.

::: {.callout-warning}
## Maturity: experimental
ferx-core labels `[diffusion]` (SDE), `[covariate_nn]` (neural networks) and parameter priors **experimental**: validated on a small set of examples, with syntax that may still change. Every fit that uses `[diffusion]` or `[covariate_nn]` carries an `experimental` warning. See [feature maturity](https://ferx-nlme.github.io/ferx-core/maturity.html).
:::
...
The bundle records the method chain as a single string where the reader expects a list of methods, so the import stops there. Until that is reconciled, write priors inline.
