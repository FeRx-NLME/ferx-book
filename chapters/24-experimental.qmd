...
`shift_in_prior_sds` is how far the estimate landed from the prior's centre, in prior standard deviations and with its sign; `penalty` is that parameter's share of `ofv_prior`. `prior_lower_95` and `prior_upper_95` are the interval that `rse = 10%` implies for the `family` the engine chose (`?ferx_fit` describes every column). An estimate `r round(abs(fit_prior$prior_summary$shift_in_prior_sds), 1)` prior standard deviations from the centre, outside that interval, says the prior and these data disagree. The standard errors of a priored fit are those of the penalized objective. Compare priored models with each other by a ΔOFV against a null twin.
