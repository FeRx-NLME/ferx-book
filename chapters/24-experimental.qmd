...
data.frame(fit = c("no prior", "prior on TVCL"),
           ...
           ofv_plus_2k = c(fit_plain$ofv + 2 * fit_plain$n_parameters,
                           fit_prior$ofv + 2 * fit_prior$n_parameters))
```

The split of the objective into its data and prior halves is not on the R fit object at this build, so the size of the penalty has to be read off that difference. Compare priored models by a ΔOFV against a null twin, not against an unpenalized fit.
