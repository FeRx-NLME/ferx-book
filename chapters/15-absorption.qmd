...
::: {.callout-important}
## `individual_estimates` of ODE models at this build
For ODE models, `fit$individual_estimates` of the pinned ferx-r build does not report all individual parameters correctly. For `warfarin_ode_lagtime`, `KA` and `LAGTIME` come out as 0:
...
Until this is fixed, list the individual parameters of an ODE model in `[output]` (@sec-tables-figures). They then appear in `fit$sdtab` with the right values, which agree with the analytical fit above to about four significant digits:

```{r}
#| label: ode-output-workaround
...
round(fit_bio$cor_matrix[c("TVCL", "TVV", "THETA_F"), c("TVCL", "TVV", "THETA_F")], 2)
```

The data contain oral doses only. With oral data, clearance, volume and `F` cannot be separated, which the correlations of 0.98 to 1 show. Fix `F` at the value used in the simulation (0.70, according to the model file) with `FIX`:
