...
Every route in those equations reads the **same** dose event, and the pathway fractions split it inside the equation. So a two-route model needs one dose record, not one per route. The shared dataset has exactly that: a single `EVID = 1` row into `CMT` 1 per subject, followed by observations.
...
Giving each route its own dose row delivers the dose twice. A copy of subject 1 with the dose row duplicated doubles the peak:
...
The peak is `r round(peaks[["two_dose_rows"]] / peaks[["one_dose_row"]], 2)` times higher, because `FR1 + FR2 = 1` applied to two dose records is two doses. When the routes start at different times, keep the single record and give one route a `lag=`, as `per_route_lag_absorption` does above.

### Bioavailability

`bioavailability` estimates the fraction `F` on the logit scale, so it stays between 0 and 1, with a random effect on that scale. It is passed with `f=` to the `pk` line. `bioavailability_ode` is the ODE version: an individual parameter named `F` is applied to the dose by the engine and does not appear in the equations. Both give the same fit:
