...
The argument order across the package is chosen so that a whole step reads as one pipeline: `ferx_model()` takes the data first, and the functions that consume a fit take the fit first. So a dataset can go to a fitted model and on to its warnings without naming an intermediate object:
...
The same holds for the other accessors and for simulation, where the model comes first:

```{r}
#| label: pipeline-more
fit |> check_diagnostics() |> _$shrinkage |> head(3)
base$model |> ferx_simulate(base$data, n_sim = 1, seed = 1) |> head(3)
```

One exception is worth knowing: `ferx_model_validate()` reads a model *file*, so a path pipes into it, as above, but a `ferx_model` object does not.
