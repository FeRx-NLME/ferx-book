...
The printed fit shows a dedicated `OMEGA_IOV  (inter-occasion variability)`
section with per-kappa CV%, SE, and shrinkage, followed by per-occasion
shrinkage in a `--- Shrinkage by occasion ---` sub-table:
...
`shrinkage_kappa_by_occ` is a data frame with one row per occasion and one
column per kappa parameter.  The `occ` column holds a 1-based occasion index:

```{r}
#| label: iov-shrinkage-by-occ
#| eval: false
...
High shrinkage (> 30%) on a particular occasion indicates that the data for
that occasion are too sparse to estimate per-occasion kappa EBEs reliably.
If shrinkage is high on all occasions, consider removing IOV from that
parameter or using a sparser model (fewer kappas).

### SAEM with IOV

`warfarin_iov_saem` uses the same DSL with `method = saem` in `[fit_options]`.
SAEM estimates kappa parameters directly — no FOCE pre-fit is required:
...
High shrinkage (> 30%) means the data cannot reliably estimate per-occasion
kappas — consider removing IOV from that parameter.  The per-occasion table
is especially useful when occasions differ in sampling density: an early
rich-sampling occasion may have low shrinkage while a later sparse occasion
has high shrinkage, pointing to where the IOV is not supported by the data.
