...
`summary(fit)` reports the auto-derived model structure (`model_type`, IIV,
residual), the gradient method used (ODE models fall back from AD to FD
automatically — visible as ` Gradient (requested): auto   (used: fd)`), and the
condition number from the covariance step.
