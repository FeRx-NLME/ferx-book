...
::: {.callout-important}
## Pass `output_model` and `output_data` at this build
At the pinned build, `ferx_model_to_frem()` does not use `output_dir`. Without `output_model` and `output_data` it writes the files next to the model, which for a `ferx_example()` model is the installed package. Give both paths explicitly.
:::

```{r}
#| label: frem-build
frem <- ferx_model_to_frem(base$model, base$data, fit = fit_base,
                           output_model = file.path(covariate_dir, "base_frem.ferx"),
                           output_data = file.path(covariate_dir, "base_frem.csv"))
frem
```
...
frem_crcl <- ferx_model_to_frem(base$model, base$data, covariates = "CRCL",
                                output_model = file.path(covariate_dir, "crcl_frem.ferx"),
                                output_data = file.path(covariate_dir, "crcl_frem.csv"))
...
- **FREM needs a suitable method.** Check the covariate–parameter correlations with SAEM, and write the FREM files outside the package directory at this build.
