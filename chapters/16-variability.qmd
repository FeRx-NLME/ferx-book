...
A partial block estimates only what it declares. The model gives `ETA_KA` no covariance with the other two, so its row and column of the omega matrix stay exactly 0, and so do their standard errors. `fit$se_omega` is the lower triangle read column by column — (1,1), (2,1), (3,1), (2,2), (3,2), (3,3) — so the third and fifth entries are the two held covariances:
