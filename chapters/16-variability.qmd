...
The last three change how the theta is parameterised, which @sec-first-fit covers -- `log` and `logit` are also *reported* on the transformed scale, while a `logit_probability` theta is reported on (0, 1). The first two do not, and are the forms this chapter uses. The three transformed rows are recognised by their exact shape: wrapping one in anything else, as in `2.0 * inv_logit(logit(TVKA) + ETA_KA)`, takes the theta back to `identity` and the eta to `custom`. The log-normal row is more forgiving, and tolerates the extra factors a covariate model puts around it.
