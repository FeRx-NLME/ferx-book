...
c(bundled = fit_w$ofv, omega_as_sd = fit_as_sd$ofv, sigma_as_variance = fit_sigma_var$ofv)
```
...
`block_omega` and `block_kappa` take variances and covariances only, because one tag cannot say which of the lower-triangle entries it applies to. ferx-core documents `(sd)` as not accepted there, but the pinned build neither rejects nor applies it -- the annotation is dropped and the numbers are read as variances:
...
Write block entries as variances, and convert by hand.
:::
