...
`ferx_fit()` refuses the same file, with the message but not the code (@sec-model-files):
...
```{r}
#| label: block-variance-tag
block_variance <- file.path(variability_dir, "warfarin_block_variance.ferx")
writeLines(sub("block_omega (ETA_CL, ETA_V) = [0.07, 0.02, 0.02]",
               "block_omega (ETA_CL, ETA_V) = [0.07, 0.02, 0.02] (variance)",
               readLines(block_ex$model), fixed = TRUE), block_variance)
invisible(capture.output(block_variance_check <- ferx_model_validate(block_variance)))
block_variance_check$diagnostics[, c("code", "suggestion")]
```
