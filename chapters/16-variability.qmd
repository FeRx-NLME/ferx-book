...
| | `omega ETA_CL ~ 0.0 FIX` | A variance held fixed (at 0: no variability on that parameter) | |
...
::: {.callout-important}
## Partial blocks at this build
The model declares no covariance for `ETA_KA`, yet the fit above reports covariances between `ETA_KA` and both other random effects. At the pinned build, a `block_omega` combined with a diagonal `omega` fits the same model as one full block. The OFV and the omega matrix are identical, while `n_parameters` counts only the declared block:

```{r}
#| label: partial-block-issue
...
c(partial_block = fit_block$ofv, full_block = fit_full_block$ofv)
c(partial_block = fit_block$n_parameters, full_block = fit_full_block$n_parameters)
all.equal(fit_block$omega, fit_full_block$omega)
```

Until this is resolved, treat a partial block as a full block. The R warning printed with the block fit above ("One or more diagonal elements are non-positive") comes from the correlation matrix of the estimates that ferx-r derives from their covariance matrix.
:::
...
try(ferx_fit(ruv_model, w$data, verbose = FALSE))
...
- **Partial omega blocks estimate a full block at this build.** See the callout above.
